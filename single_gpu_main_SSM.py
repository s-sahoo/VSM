...
parser.add_argument("--mamba", default='', type=str, required=False, 
                    choices=['', 'mamba', 'mamba_BD', 'mamba_BD_outFFN'],
...
parser.add_argument("--from_scratch", default=0, type=int, required=False)
...
save_path = "./saved_models/" + wandb_name + "_ckpts"
...
print(f"Performing Model Surgery for Mamba")
...
train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, collate_fn=collate_fn, num_workers=0, pin_memory=True)
dev_loader = torch.utils.data.DataLoader(dev_data, batch_size=args.batch_size * 2, collate_fn=collate_fn, num_workers=0, pin_memory=True)
